...
void D3D12Engine::SwapChain::Initialize(
  ...
  swapChainDescriptor.BufferCount = GraphicsCore::FrameCount;
  ...
  for (UINT frame = 0; frame < GraphicsCore::FrameCount; frame += 1) {
...
void D3D12Engine::SwapChain::Present(CommandQueue& rCmdQueue) {
  m_fenceValues[m_frameIndex] = rCmdQueue.Signal();

  m_swapChain->Present(1, 0);

  m_frameIndex = m_swapChain->GetCurrentBackBufferIndex();

  rCmdQueue.WaitForPreviousFrame(m_fenceValues[m_frameIndex]);
}
