...
void D3D12Engine::GPUBuffer::Create(ID3D12Device* device, const std::wstring& name, UINT sizeInBytes, const void* initialData) {
  Destroy();
...
void D3D12Engine::GPUBuffer::UpdateUploadData(const void* data, UINT sizeInBytes) {
  ...
  m_Resource->Map(0, &readRange, reinterpret_cast<void**>(&pCBVDataBegin));
