...
D3D12Engine::InterfaceDirectX12::InterfaceDirectX12(UINT m_WindowHeight, UINT m_WindowWidht, std::wstring m_WindowName) :
  WindowHeight(m_WindowHeight), 
  WindowWidth(m_WindowWidht),
  ...
  bool requestHighPerfomanceAdpter)
{
  *ptr_Adapter1 = nullptr;
  ...
  if (SUCCEEDED(ptr_Factory1->QueryInterface(IID_PPV_ARGS(&m_Factory6)))) {
    ...
    for (UINT AdapterIndex = 0;
      SUCCEEDED(m_Factory6->EnumAdapterByGpuPreference(AdapterIndex, GpuPreference, IID_PPV_ARGS(&m_Adapter1)));
      AdapterIndex += 1)
    {
      DXGI_ADAPTER_DESC1 AdapterDescriptor;
      m_Adapter1->GetDesc1(&AdapterDescriptor);

      if (AdapterDescriptor.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) {
        continue;
      }
      if (SUCCEEDED(D3D12CreateDevice(m_Adapter1.Get(), D3D_FEATURE_LEVEL_11_0, _uuidof(ID3D12Device), nullptr))) {
        *ptr_Adapter1 = m_Adapter1.Detach();
        return;
      }
    }
  }
  ...
  for (UINT AdapterIndex = 0;
    SUCCEEDED(ptr_Factory1->EnumAdapters1(AdapterIndex, &m_Adapter1));
    AdapterIndex += 1) 
  {
